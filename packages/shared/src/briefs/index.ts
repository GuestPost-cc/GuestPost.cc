...
const guestPostBriefSchema = z.object({
...
const nicheEditBriefSchema = z.object({
  kind:              z.literal("NICHE_EDIT"),
  existingArticleUrl: targetUrlSchema,
  targetUrl:         targetUrlSchema,
  anchorText:        anchorTextSchema,
  placementContext:  makeText("Placement context", { min: 1, max: 1000, required: false }),
  notes:             notesSchema,
}).strict()
...
const editorialLinkBriefSchema = z.object({
  kind:               z.literal("EDITORIAL_LINK"),
  targetUrl:          targetUrlSchema,
  anchorText:         anchorTextSchema,
  topicalRelevance:   makeText("Topical relevance", { min: 10, max: 1000 }),
  preferredPlacement: makeText("Preferred placement", { min: 1, max: 500, required: false }),
  notes:              notesSchema,
}).strict()
...
const outreachLinkBriefSchema = z.object({
...
const localCitationBriefSchema = z.object({
  ...
  categoryHint: makeText("Business category", { min: 1, max: 120, required: false }),
  hours:        makeText("Hours of operation", { min: 1, max: 500, required: false }),
  notes:        notesSchema,
}).strict()

// ── FOUNDATION_LINK — profile / Web 2.0 / forum-style backlinks; the
// platforms array hints which networks to target.
const foundationLinkBriefSchema = z.object({
...
const blogArticleBriefSchema = z.object({
...
const seoContentBriefSchema = z.object({
...
export function validateBrief(serviceType: string, data: unknown): BriefData {
  ...
  const withKind = (data && typeof data === "object")
