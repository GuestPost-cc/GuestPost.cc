...
export interface SentryScopeLike {
  setTag: (key: string, value: string | number | boolean | null | undefined) => unknown
}

// Apply a business context to a Sentry scope. Pass only defined keys so we
// never clobber a useful tag with `undefined`.
export function setBusinessContext(scope: SentryScopeLike, ctx: BusinessContext): void {
  for (const [key, value] of Object.entries(ctx) as Array<[keyof BusinessContext, unknown]>) {
