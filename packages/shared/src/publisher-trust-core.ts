...
export const TRUST_RECOMPUTE_DEBOUNCE_MS = Number(process.env.TRUST_RECOMPUTE_DEBOUNCE_MS) || 5000
...
async function notifyOps(prisma: any, type: string, message: string) {
  ...
  for (const s of staff) {
    await prisma.notification.create({ data: { userId: s.userId, organizationId: null, type, message } }).catch(() => undefined)
  }
}
...
export async function recomputePublisherTrustCore(
  prisma: any,
  publisherId: string,
  opts: { sourceEvent: string; reason?: string; actorUserId?: string | null } = { sourceEvent: "MANUAL" },
  ...
  const [reviewAgg, totalOrders, completedOrders, disputeCount, refundCount, linkRemovals, websiteRevocations] = await Promise.all([
    prisma.orderReview.aggregate({ where: { publisherId }, _avg: { rating: true }, _count: { _all: true } }),
    prisma.order.count({ where: { website: { publisherId } } }),
    prisma.order.count({ where: { website: { publisherId }, status: { in: ["DELIVERED", "SETTLED", "COMPLETED"] } } }),
    prisma.orderDispute.count({ where: { order: { website: { publisherId } } } }),
    prisma.order.count({ where: { website: { publisherId }, status: "REFUNDED" } }),
    prisma.deliveryFraudFlag.count({ where: { type: "LINK_REMOVED", order: { website: { publisherId } } } }),
    prisma.website.count({ where: { publisherId, verificationStatus: "REVOKED" } }),
  ])
  ...
  if (changed) {
    await prisma.publisher.update({ where: { id: publisherId }, data: { tier } }).catch(() => undefined)
  }
  ...
  await prisma.auditLog.create({
    data: { action: "PUBLISHER_TRUST_RECOMPUTED", entityType: "Publisher", entityId: publisherId, metadata: meta, userId: opts.actorUserId ?? null, organizationId: null },
  })

  if (changed) {
    const direction = scoreTierRank(tier) > scoreTierRank(oldTier) ? "upgraded" : "downgraded"
    await prisma.auditLog.create({
      data: { action: "PUBLISHER_TIER_CHANGED", entityType: "Publisher", entityId: publisherId, metadata: { ...meta, direction }, userId: opts.actorUserId ?? null, organizationId: null },
    })
    await notifyOps(prisma, "PUBLISHER_TIER_CHANGED", `Publisher ${publisher.name ?? publisherId} ${direction}: ${oldTier} → ${tier} (trust ${oldScore ?? "?"} → ${score}, via ${opts.sourceEvent})`)
  }

  // Observability — single structured line carries the "metrics" fields.
  console.log(`[TRUST] recompute publisher=${publisherId} source=${opts.sourceEvent} score=${oldScore}->${score} tier=${oldTier}->${tier} changed=${changed} durationMs=${durationMs}`)

  return { publisherId, oldScore, newScore: score, oldTier, newTier: tier, changed, durationMs }
}
