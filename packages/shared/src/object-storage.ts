...
function getClient(): { client: S3Client; bucket: string } {
  ...
  const endpoint =
    ...
    (process.env.MINIO_ENDPOINT ? `http://${process.env.MINIO_ENDPOINT}` : undefined)
  const accessKeyId =
    process.env.S3_ACCESS_KEY ?? process.env.R2_ACCESS_KEY_ID ?? process.env.MINIO_ACCESS_KEY ?? ""
  const secretAccessKey =
    process.env.S3_SECRET_KEY ?? process.env.R2_SECRET_ACCESS_KEY ?? process.env.MINIO_SECRET_KEY ?? ""
  bucket = process.env.S3_BUCKET ?? process.env.R2_BUCKET ?? process.env.MINIO_BUCKET ?? "guestpost"
...
export async function presignGet(objectKey: string, expiresInSeconds = 900): Promise<string> {
  const { client, bucket } = getClient()
