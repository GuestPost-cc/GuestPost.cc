...
describe("EmailVerificationBanner", () => {
  it("renders the email and the default 'Resend email' label when ready", () => {
    ...
    render(<EmailVerificationBanner email="alice@example.com" onResend={onResend} />)
  ...
  it("renders 'Sending…' and disables the button while sending", () => {
    const onResend = vi.fn()
    render(<EmailVerificationBanner email="alice@example.com" sending onResend={onResend} />)
  ...
  it("renders 'Resend in Ns' and disables the button during cooldown", () => {
    const onResend = vi.fn()
    render(
      <EmailVerificationBanner email="alice@example.com" cooldownSeconds={42} onResend={onResend} />
    )
  ...
  it("renders the role=status semantics for assistive tech", () => {
    render(<EmailVerificationBanner email="alice@example.com" onResend={() => {}} />)
  ...
  it("honors a custom message override", () => {
    ...
    expect(screen.getByText("Custom copy for a different verification gate.")).toBeInTheDocument()
