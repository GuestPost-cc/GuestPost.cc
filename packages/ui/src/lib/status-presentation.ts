...
export const ORDER_STATUS_PRESENTATION: Record<OrderStatus, StatusPresentation> = {
  ...
  PENDING_PAYMENT:   { variant: "warning",     label: "Pending Payment",  chartColor: "#f59e0b" },
  ...
  CONTENT_REQUESTED: { variant: "info",        label: "Content Requested", chartColor: "#3b82f6" },
  CONTENT_CREATION:  { variant: "info",        label: "Content Creation", chartColor: "#3b82f6" },
  CONTENT_READY:     { variant: "info",        label: "Content Ready",    chartColor: "#3b82f6" },
  CUSTOMER_REVIEW:   { variant: "warning",     label: "Customer Review",  chartColor: "#f59e0b" },
  ...
  CANCELLED:         { variant: "destructive", label: "Cancelled",        chartColor: "#ef4444" },
  REFUNDED:          { variant: "destructive", label: "Refunded",         chartColor: "#ef4444" },
  DISPUTED:          { variant: "destructive", label: "Disputed",         chartColor: "#ef4444" },
}

// ─── Ticket (5 states) — `OPEN` is INFO (blue) by deliberate choice ────────

export const TICKET_STATUS_PRESENTATION: Record<TicketStatus, StatusPresentation> = {
  OPEN:                 { variant: "info",    label: "Open",                  chartColor: "#3b82f6" },
  IN_PROGRESS:          { variant: "warning", label: "In Progress",           chartColor: "#f59e0b" },
  WAITING_ON_CUSTOMER:  { variant: "pending", label: "Waiting on Customer",   chartColor: "#a78bfa" },
...
export const DISPUTE_STATUS_PRESENTATION: Record<DisputeStatus, StatusPresentation> = {
  OPEN:               { variant: "destructive", label: "Open",               chartColor: "#ef4444" },
  UNDER_REVIEW:       { variant: "warning",     label: "Under Review",       chartColor: "#f59e0b" },
  RESOLVED_REFUNDED:  { variant: "info",        label: "Resolved (Refund)",  chartColor: "#3b82f6" },
  RESOLVED_REJECTED:  { variant: "pending",     label: "Resolved (Rejected)", chartColor: "#9ca3af" },
  RESOLVED_RESTORED:  { variant: "success",     label: "Resolved (Restored)", chartColor: "#10b981" },
}

// ─── Listing (6 states) ─────────────────────────────────────────────────────

export const LISTING_STATUS_PRESENTATION: Record<ListingStatus, StatusPresentation> = {
  DRAFT:           { variant: "pending",     label: "Draft",           chartColor: "#9ca3af" },
  PENDING_REVIEW:  { variant: "warning",     label: "Pending Review",  chartColor: "#f59e0b" },
  APPROVED:        { variant: "success",     label: "Approved",        chartColor: "#10b981" },
  REJECTED:        { variant: "destructive", label: "Rejected",        chartColor: "#ef4444" },
...
export const CAMPAIGN_STATUS_PRESENTATION: Record<CampaignStatus, StatusPresentation> = {
...
export function getTicketStatusPresentation(s: TicketStatus): StatusPresentation {
  return TICKET_STATUS_PRESENTATION[s]
}
export function getDisputeStatusPresentation(s: DisputeStatus): StatusPresentation {
  return DISPUTE_STATUS_PRESENTATION[s]
}
export function getListingStatusPresentation(s: ListingStatus): StatusPresentation {
  return LISTING_STATUS_PRESENTATION[s]
}
export function getCampaignStatusPresentation(s: CampaignStatus): StatusPresentation {
