...
const USERS = [
  ...
  },
  {
    ...
    password: process.env.SEED_PUBLISHER_PASSWORD ?? "Publisher123!",
    name: "John Publisher",
    type: "PUBLISHER",
  ...
  },
  {
    ...
    password: process.env.SEED_MEMBER_PASSWORD ?? "Member123!",
    name: "Mike Member",
    type: "CUSTOMER",
    role: "MEMBER",
  },
...
async function seed() {
  ...
  if (!adminSignin.ok)
    throw new Error(`Admin signin failed: ${await adminSignin.text()}`)
  ...
  for (const u of USERS) {
    ...
    if (u.type === "STAFF") {
      const staffRes = await fetch(
        `${API}/api/v1/admin/users/${target.id}/staff-role`,
        {
          method: "PATCH",
          headers: {
      ...
      const roleRes = await fetch(
        `${API}/api/v1/admin/users/${target.id}/role`,
        {
          method: "PATCH",
          headers: {
            ...authHeaders(adminToken),
  ...
  for (const u of USERS.filter((x) => x.type === "CUSTOMER")) {
    ...
    if (orgs.length === 0 && u.role === "OWNER") {
      const orgRes = await fetch(`${API}/api/v1/identity/organizations`, {
        method: "POST",
        headers: {
          ...authHeaders(custData.token),
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          name: `${target.name}'s Agency`,
          slug: `${target.name?.toLowerCase().replace(/\s+/g, "-")}-agency`,
        }),
      })
      if (orgRes.ok) {
  ...
  const adminTarget = users.find(
    (x: any) => x.email === "admin@guestpost.local",
  )
  if (adminTarget) {
    const staffRes = await fetch(
      `${API}/api/v1/admin/users/${adminTarget.id}/staff-role`,
      {
        method: "PATCH",
        headers: {
          ...authHeaders(adminToken),
  ...
  if (!process.env.CI && !process.env.SEED_SKIP_WARNING) {
    ...
    console.log(
      "   Set SEED_*_PASSWORD env vars or SEED_SKIP_WARNING=1 to suppress this.",
    )
  }
  console.log("\nSeeding complete")
}
