...
async function main() {
  ...
  if (!existsSync(KEY_FILE)) {
    ...
    console.log(
      `Generated validation keypair. Set in .env.development:\nWISE_WEBHOOK_PUBLIC_KEY="${publicKey.replace(/\n/g, "\\n")}"\nthen restart the API and re-run.`,
    )
    process.exit(2)
  }
  const wisePrivate = readFileSync(KEY_FILE, "utf8")
  const stripeSecret = process.env.STRIPE_WEBHOOK_SECRET
  const stripePayoutSecret =
    process.env.STRIPE_PAYOUT_WEBHOOK_SECRET ?? stripeSecret
  if (!stripeSecret)
    throw new Error("STRIPE_WEBHOOK_SECRET missing from .env.development")
  ...
  const sessionPayload = {
    ...
    data: {
      object: {
        ...
        metadata: {
          walletId: wallet0.id,
          organizationId: wallet0.organizationId,
          userId: "validation",
        },
      },
    },
  ...
  const dep1 = await fireStripe(
    "/billing/webhook/stripe",
    stripeSecret,
    sessionPayload,
  )
  check(
    "deposit webhook accepted (200)",
    dep1.status === 200 || dep1.status === 201,
    dep1,
  )
  ...
  check(
    ...
    Math.abs(
      Number(wallet1.availableBalance) -
  ...
  const depRow = await prisma.transaction.findFirst({
    where: { reference: sessionId },
  })
  check(
    "ledger row written with payment_intent linkage",
    depRow?.providerRef === piId,
    depRow,
  )

  // Duplicate delivery (Stripe retries are routine)
  const dep2 = await fireStripe(
    "/billing/webhook/stripe",
    stripeSecret,
    sessionPayload,
  )
  check(
    "duplicate deposit webhook accepted (replay-safe 200)",
    dep2.status === 200 || dep2.status === 201,
    dep2,
  )
  const wallet2 = (await call("GET", "/billing/wallet", client)).data
  check(
    "duplicate deposit did NOT double-credit",
    Number(wallet2.availableBalance) === Number(wallet1.availableBalance),
    {
      after1: wallet1.availableBalance,
      after2: wallet2.availableBalance,
    },
  )
  ...
  check(
    "tampered deposit signature rejected (400)",
    badSig.status === 400,
    badSig.status,
  )
  ...
  // ════════════════════════ STRIPE: CHARGEBACK ══════════════════════════════
  console.log(
    "── Stripe chargeback lifecycle (hold → won release; hold → lost debit)",
  )
  ...
  check(
    "dispute.created accepted",
    cb1.status === 200 || cb1.status === 201,
    cb1,
  )
  ...
  check(
    ...
    Math.abs(
      Number(wallet2.availableBalance) - Number(wallet3.availableBalance) - 50,
    ) < 0.001 &&
      Math.abs(
        Number(wallet3.reservedBalance) - Number(wallet2.reservedBalance) - 50,
      ) < 0.001,
    {
      av2: wallet2.availableBalance,
      av3: wallet3.availableBalance,
      rs2: wallet2.reservedBalance,
      rs3: wallet3.reservedBalance,
    },
  )
  ...
  check(
    "duplicate dispute webhook did not double-hold",
    Number(wallet3b.reservedBalance) === Number(wallet3.reservedBalance),
    {
      rs: wallet3.reservedBalance,
      rsDup: wallet3b.reservedBalance,
    },
  )
  ...
  check(
    ...
    Math.abs(
      Number(wallet4.availableBalance) - Number(wallet2.availableBalance),
    ) < 0.001 &&
      Math.abs(
        Number(wallet4.reservedBalance) - Number(wallet2.reservedBalance),
      ) < 0.001,
  ...
  check(
    ...
    Math.abs(
      Number(wallet4.availableBalance) -
        50 -
        (Number(wallet5.availableBalance) +
          Number(wallet5.reservedBalance) -
          Number(wallet4.reservedBalance)),
    ) < 0.001 &&
      Math.abs(
        Number(wallet5.reservedBalance) - Number(wallet4.reservedBalance),
      ) < 0.001,
    {
      av4: wallet4.availableBalance,
      av5: wallet5.availableBalance,
      rs4: wallet4.reservedBalance,
      rs5: wallet5.reservedBalance,
    },
  )
  const lostRow = await prisma.transaction.findFirst({
    where: { reference: `chargeback-lost-${disputeLost}` },
  })
  check(
    "CHARGEBACK ledger row written for lost dispute",
    !!lostRow && Number(lostRow.amount) === -50,
    lostRow?.amount,
  )

  // ════════════════════════ WISE: WEBHOOK PATH ══════════════════════════════
  console.log(
    "── Wise webhook path (real RSA-SHA256 verification, real worker transitions)",
  )
  ...
  const wiseProvider = await prisma.payoutProvider.findUnique({
    where: { name: "wise" },
  })
  check("wise provider registered", !!wiseProvider)
  const pub = await prisma.publisher.findFirstOrThrow({
  ...
  const exec = await prisma.payoutExecution.create({
    data: {
      ...
      status: "PROCESSING",
      providerExecutionId: String(transferId),
      amount: 25,
      idempotencyKey: `payout-${wd.id}-v1`,
    },
  })
  const lifetimeBefore = Number(
    (
  ...
  check(
    "tampered Wise signature rejected (401)",
    tampered.status === 401,
    tampered.status,
  )
  ...
  check(
    "non-terminal Wise state accepted",
    sent.status === 200 || sent.status === 201,
    sent,
  )
  await sleep(2500)
  const execMid = await prisma.payoutExecution.findUniqueOrThrow({
    where: { id: exec.id },
  })
  check(
    "non-terminal state caused no transition",
    execMid.status === "PROCESSING",
    execMid.status,
  )
  ...
  check(
    "completed Wise webhook accepted",
    done.status === 200 || done.status === 201,
    done,
  )

  const completed = await waitFor(async () => {
    const e = await prisma.payoutExecution.findUniqueOrThrow({
      where: { id: exec.id },
    })
    return e.status === "COMPLETED" ? e : null
  })
  check("worker completed the execution via webhook", !!completed)
  const wdAfter = await prisma.withdrawal.findUniqueOrThrow({
    where: { id: wd.id },
  })
  check(
    "withdrawal transitioned to COMPLETED",
    wdAfter.status === "COMPLETED",
    wdAfter.status,
  )
  const lifetimeAfter = Number(
    (
      await prisma.publisherBalance.findUniqueOrThrow({
        where: { publisherId: pub.id },
      })
  ...
  check(
    "lifetimePaid incremented exactly $25",
    Math.abs(lifetimeAfter - lifetimeBefore - 25) < 0.001,
  ...
  check(
    "replayed Wise webhook did NOT double-pay",
    lifetimeReplay === lifetimeAfter,
    { lifetimeAfter, lifetimeReplay },
  )
  ...
  const exec2 = await prisma.payoutExecution.create({
    data: {
      ...
      providerId: wiseProvider?.id,
      status: "PROCESSING",
      providerExecutionId: String(transferId2),
      amount: 10,
      idempotencyKey: `payout-${wd2.id}-v1`,
    },
  })
  await fireWise(
    {
      data: {
        resource: { id: transferId2, type: "transfer" },
  ...
  const wd2After = await prisma.withdrawal.findUniqueOrThrow({
    where: { id: wd2.id },
  })
  check(
    "withdrawal marked FAILED (reversal path now available)",
    wd2After.status === "FAILED",
    wd2After.status,
  )
  const lifetimeFail = Number(
    (
      await prisma.publisherBalance.findUniqueOrThrow({
        where: { publisherId: pub.id },
      })
  ...
  check("failed transfer paid nothing", lifetimeFail === lifetimeAfter, {
    lifetimeFail,
  })
  ...
  const reversed = await call(
    "POST",
    `/admin/withdrawals/${wd2.id}/reverse`,
    admin,
    { reason: "provider cancelled transfer — validation run" },
  )
  check(
    "FAILED withdrawal reversed via admin API",
    reversed.status < 300 && reversed.data.status === "REVERSED",
    reversed.data?.status ?? reversed.status,
  )

  // ════════════════════════ STRIPE PAYOUT WEBHOOK ═══════════════════════════
  console.log("── Stripe payout webhook (transfer paid)")
  const trId = `tr_val_${Date.now()}`
  const stripeProvider = await prisma.payoutProvider.findUnique({
    where: { name: "stripe_connect" },
  })
  ...
  const exec3 = await prisma.payoutExecution.create({
    data: {
      ...
      status: "PROCESSING",
      providerExecutionId: trId,
      amount: 15,
      idempotencyKey: `payout-${wd3.id}-v1`,
    },
  })
  const sp = await fireStripe(
    "/payout-webhooks/stripe_connect",
    stripePayoutSecret!,
    {
      id: `evt_tr_${Date.now()}`,
      type: "transfer.updated",
      data: {
        object: { id: trId, object: "transfer", status: "paid", amount: 1500 },
      },
    },
  )
  check(
    "stripe payout webhook accepted",
  ...
  check("reconciliation after all provider events: ok", recon.ok === true, {
    walletDrift: recon.walletDrift?.length,
    publisherDrift: recon.publisherDrift?.length,
    stuckPayouts: recon.stuckPayouts?.length,
  })
