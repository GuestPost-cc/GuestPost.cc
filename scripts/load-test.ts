...
async function signIn(email: string, password: string) {
  ...
  if (r.status !== 200) throw new Error(`sign-in failed: ${email} ${JSON.stringify(r.data)}`)
...
async function provisionUserWithSession(email: string, name: string, slug: string): Promise<string> {
  ...
  await prisma.activeContext.create({ data: { userId: user.id, activeOrganizationId: org.id } })
...
async function fundWalletDirect(orgId: string, userId: string, amount: number, reference: string) {
  await prisma.$transaction(async (tx: any) => {
    const wallet = await tx.wallet.upsert({
      where: { organizationId: orgId },
      create: { organizationId: orgId, userId, availableBalance: amount, reservedBalance: 0, currency: "USD" },
      update: { availableBalance: { increment: amount } },
    })
    await tx.transaction.create({
      data: { walletId: wallet.id, amount, type: "DEPOSIT", description: `Deposit ${reference}`, reference },
    })
  })
}
...
async function main() {
  ...
  const site = await prisma.website.findFirstOrThrow({ where: { domain: "techinsider.example.com" } })
  ...
  await pool(ids, CONCURRENCY, async (i) => {
    ...
    const { token, orgId } = await provisionUserWithSession(email, `Load User ${i}`, `load-${runId}-${i}`)
    const userId = (await prisma.user.findUniqueOrThrow({ where: { email } })).id
    await fundWalletDirect(orgId, userId, price, `load-${runId}-${i}`)
    users[i] = { token, email }
  })
  console.log(`   provisioned ${users.length} users in ${((Date.now() - provisionStart) / 1000).toFixed(1)}s`)
  ...
  await pool(users, CONCURRENCY, async (u) => {
    const t0 = Date.now()
    try {
      const order = (await call("POST", "/orders", u.token, {
        type: "GUEST_POST",
        title: `load order ${runId}`,
        items: [{ websiteId: site.id, targetUrl: "https://example.com/load", anchorText: "load" }],
      })).data
      const pay = await call("POST", `/orders/${order.id}/submit-payment`, u.token)
  ...
  console.log("\n── Integrity checks")
  const paidOrders = await prisma.order.count({ where: { title: `load order ${runId}`, status: { in: ["PAID", "SUBMITTED"] } } })
  const purchaseAgg = await prisma.transaction.aggregate({
    where: { type: "PURCHASE", description: { contains: "" }, order: { title: `load order ${runId}` } },
    _sum: { amount: true },
    _count: true,
  })
  ...
  expect("paid order count equals OK responses", paidOrders === ok, { paidOrders, ok })
  expect("one PURCHASE transaction per paid order", purchaseAgg._count === paidOrders, { purchases: purchaseAgg._count, paidOrders })
  expect("total debited equals paidOrders * price", Math.abs(totalDebited - paidOrders * price) < 0.001, { totalDebited, expected: paidOrders * price })

  const recon = (await call("GET", "/admin/reconciliation", await signIn("admin@guestpost.local", "Admin123!"))).data
  expect("reconciliation: zero drift after storm", recon.ok === true, recon.ok ? undefined : { wallet: recon.walletDrift?.length, pub: recon.publisherDrift?.length })
