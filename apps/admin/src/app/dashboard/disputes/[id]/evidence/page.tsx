      ...
      {data.dispute && (
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              Status: <Badge variant="outline">{data.dispute.status}</Badge>
            </div>
        ...
        <Card className="border-destructive">
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2 text-destructive">
              <ShieldAlert className="h-4 w-4" /> Fraud Flags
            </CardTitle>
          </CardHeader>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <FileText className="h-4 w-4" /> Delivery Versions
          </CardTitle>
        </CardHeader>
              ...
              <div key={v.id} className="rounded-lg border p-3 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">
                    v{v.version}
                    {v.supersededByVersion
                      ? ` → superseded by v${v.supersededByVersion}`
                      : " (active)"}
                  </span>
                  <Badge
                    variant={
                ...
                {ev && (
                  <div className="grid grid-cols-2 gap-1 text-xs text-muted-foreground">
                    ...
                    <span>Link: {ev.linkFound ? "✓" : "✗"}</span>
                    <span>Target: {ev.targetUrlMatched ? "✓" : "✗"}</span>
          ...
          {data.snapshots?.map((s: any) => (
            ...
            >
              <span className="text-muted-foreground">
                v{s.version} · {format(new Date(s.createdAt), "PPp")}
              </span>
