...
export function splitPlatformFee(gross: Decimal | number | string, feeFraction: number): { fee: Decimal; net: Decimal } {
  const g = new Decimal(gross)
  const fee = g.mul(new Decimal(feeFraction)).toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
