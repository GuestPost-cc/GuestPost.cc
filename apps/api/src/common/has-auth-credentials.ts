...
// `${prefix}-${name}`, with an optional `__Secure-` prefix in production.
// Our config sets prefix=guestpost, name defaults to session_token.
//
...
const SESSION_COOKIE_RE =
  /(?:^|;\s*)(?:__Secure-)?guestpost[.-]session_token=([^;]+)/
