...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const sensitiveRequired = requiredPermissions.filter((p) => SENSITIVE_PERMISSIONS.includes(p))
    if (user.staffRole === "SUPER_ADMIN" && sensitiveRequired.length === 0) return true
    ...
    const toCheck = user.staffRole === "SUPER_ADMIN" ? sensitiveRequired : requiredPermissions
    const hasAll = toCheck.every((p) => userPermissions.includes(p))
    if (!hasAll) throw new ForbiddenException("Missing required permission: " + toCheck.join(", "))
    return true
  }
}