...
function aggregate(sum: { amount: number; fee: number; net: number }, count: number) {
  return {
    _sum: {
...
describe("Phase 7.1 — RevenueService", () => {
    ...
    it("returns PLATFORM + PUBLISHER buckets with correct sums and sorted ASC", async () => {
      ...
      prisma.platformRevenue.groupBy
        ...
        .mockResolvedValueOnce([
          { fulfillmentChannel: "PLATFORM", ...aggregate({ amount: 200, fee: 20, net: 180 }, 2) },
          { fulfillmentChannel: "PUBLISHER", ...aggregate({ amount: 300, fee: 30, net: 270 }, 3) },
        ])
        // Reversed counts
        .mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
  ...
  describe("groupBy=serviceType — NULL snapshot bucket", () => {
    it("collapses NULL serviceType into '(unknown)' bucket without crashing", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { serviceType: "GUEST_POST", ...aggregate({ amount: 100, fee: 10, net: 90 }, 1) },
          { serviceType: null, ...aggregate({ amount: 50, fee: 5, net: 45 }, 1) },
        ])
        .mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
      ...
      expect(r.buckets.map((b) => b.bucketKey).sort()).toEqual(["(unknown)", "GUEST_POST"])
  ...
  describe("groupBy=listing — joined human title + sorted by netRevenue DESC", () => {
    it("joins ListingService → MarketplaceListing and populates structured fields", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { listingServiceId: "lst-A", ...aggregate({ amount: 100, fee: 10, net: 90 }, 1) },
          { listingServiceId: "lst-B", ...aggregate({ amount: 500, fee: 50, net: 450 }, 3) },
          { listingServiceId: "lst-C", ...aggregate({ amount: 200, fee: 20, net: 180 }, 2) },
        ])
        .mockResolvedValueOnce([])
      ...
      expect(r.buckets.map((b) => b.bucketKey)).toEqual(["lst-B", "lst-C", "lst-A"])
    ...
    it("preserves bucket when ListingService→Listing join misses (soft-deleted)", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { listingServiceId: "lst-ghost", ...aggregate({ amount: 100, fee: 10, net: 90 }, 1) },
        ])
        .mockResolvedValueOnce([])
    ...
    it("listing grouping with NULL listingServiceId → '(unknown)' with null structured fields", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { listingServiceId: null, ...aggregate({ amount: 100, fee: 10, net: 90 }, 1) },
        ])
        .mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
  ...
  describe("reversed row handling", () => {
    it("excludes reversed rows from sums; surfaces reversedCount separately", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { fulfillmentChannel: "PLATFORM", ...aggregate({ amount: 300, fee: 30, net: 270 }, 3) },
        ])
        // 2 reversed rows for the same bucket
        .mockResolvedValueOnce([{ fulfillmentChannel: "PLATFORM", _count: { _all: 2 } }])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
    ...
    it("preserves a reversed-only bucket (no non-reversed rows) so it isn't lost", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([{ fulfillmentChannel: "PLATFORM", _count: { _all: 5 } }])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
  ...
  describe("Decimal precision", () => {
    it("0.10 fee on a 1000.00 gross serializes as '100.00'", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { fulfillmentChannel: "PLATFORM", ...aggregate({ amount: 1000, fee: 100, net: 900 }, 1) },
        ])
        .mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
  ...
  describe("previous-period comparison", () => {
    it("computes a same-duration previous window when both from + to are set", async () => {
      ...
      prisma.platformRevenue.groupBy
        .mockResolvedValueOnce([
          { fulfillmentChannel: "PLATFORM", ...aggregate({ amount: 1200, fee: 120, net: 1080 }, 1) },
        ])
        .mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate
        // Current totals
        .mockResolvedValueOnce(aggregate({ amount: 1200, fee: 120, net: 1080 }, 1))
        // Previous totals
        .mockResolvedValueOnce(aggregate({ amount: 1000, fee: 100, net: 900 }, 1))
    ...
    it("returns deltaPct null when previous gross is 0 (avoids Infinity / NaN)", async () => {
      const prisma = makePrismaMock()
      prisma.platformRevenue.groupBy.mockResolvedValueOnce([]).mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate
        .mockResolvedValueOnce(aggregate({ amount: 1000, fee: 100, net: 900 }, 1))
  ...
  describe("currency mismatch — queries Order, not PlatformRevenue", () => {
    it("populates meta.currencyMismatch when non-USD Orders exist in range", async () => {
      ...
      prisma.platformRevenue.groupBy.mockResolvedValueOnce([]).mockResolvedValueOnce([])
      prisma.platformRevenue.aggregate.mockResolvedValue(zeroAggregate)
      prisma.order.findMany.mockResolvedValue([{ currency: "EUR" }, { currency: "GBP" }])
      ...
      expect(r.meta.currencyMismatch!.distinctCurrencies).toEqual(["EUR", "GBP"])
    })
