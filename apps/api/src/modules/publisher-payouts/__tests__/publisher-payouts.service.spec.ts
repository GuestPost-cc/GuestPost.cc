...
describe("PublisherPayoutsService", () => {
  ...
  describe("requestWithdrawal", () => {
    it("sets availableAt from tier hold and writes a WITHDRAWAL ledger row", async () => {
      const created = { id: "wd-1", publisherId: "pub-1", amount: new Decimal(100) }
      ...
      expect(Math.abs(createCall.data.availableAt.getTime() - expectedMs)).toBeLessThan(5000)
    ...
    it("returns existing withdrawal on idempotency key replay without moving balance", async () => {
      ...
      const result = await service.requestWithdrawal("pub-1", 100, "bank_transfer", "user-1", "key-1")
  ...
  describe("approveWithdrawal", () => {
    it("rejects approval while tier hold is active", async () => {
      ...
      await expect(service.approveWithdrawal("wd-1", "staff-1")).rejects.toThrow(/tier hold/)
    ...
    it("approves once the hold has elapsed", async () => {
      ...
      prismaMock.withdrawal.findUniqueOrThrow.mockResolvedValue({ id: "wd-1", status: "APPROVED" })
      ...
      expect(prismaMock.withdrawal.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: "wd-1", status: "PENDING", version: 0 } }),
      )
    })
  })
  ...
  describe("rejectWithdrawal", () => {
    it("restores balance and writes WITHDRAWAL_REVERSAL ledger row", async () => {
      ...
      prismaMock.withdrawal.findUniqueOrThrow.mockResolvedValue({ id: "wd-1", status: "REJECTED" })
      ...
      expect(prismaMock.publisherBalance.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ withdrawableBalance: { increment: 100 } }),
        }),
      )
    })
  })
