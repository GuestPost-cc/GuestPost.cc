...
export class DeliveryInterventionService {
  ...
  async reverify(deliveryVersionId: string, userId: string) {
    ...
    await this.audit.log({
      ...
      entityType: "OrderDeliveryVersion",
      entityId: version.id,
      metadata: this.deliveryAuditMeta(order, version, { reverify: true }),
      userId,
      organizationId: order.organizationId,
    })
    await this.queue.addJob(
      ...
      "delivery-verify",
      { deliveryVersionId: version.id, actorUserId: userId },
      {
        jobId: `delivery-verify-${version.id}-${Date.now()}`,
        attempts: 3,
        backoff: { type: "custom" },
      },
    )
  ...
  async requestRevision(
    ...
    if (!["PUBLISHED", "VERIFIED"].includes(order.status)) {
      throw new BadRequestException(
        "Revisions can only be requested on a delivered order",
      )
    }
    return this.prisma.$transaction(async (tx: any) => {
      ...
      await this.audit.log(
        {
          action: "ORDER_DELIVERY_REVISION_REQUESTED",
          entityType: "Order",
          entityId: orderId,
          metadata: this.deliveryAuditMeta(
            order,
            {
              id: order.activeDeliveryVersionId ?? "",
