...
const statusConfig: Record<string, { color: string; icon: React.ElementType; description: string }> = {
  OPEN: { color: "bg-blue-100 text-blue-700", icon: AlertCircle, description: "Ticket is open and awaiting response" },
  IN_PROGRESS: { color: "bg-amber-100 text-amber-700", icon: Clock, description: "Our team is working on this" },
  WAITING_ON_CUSTOMER: { color: "bg-purple-100 text-purple-700", icon: Clock, description: "Waiting for your response" },
  RESOLVED: { color: "bg-green-100 text-green-700", icon: CheckCircle, description: "This issue has been resolved" },
  CLOSED: { color: "bg-gray-100 text-gray-500", icon: CheckCircle, description: "This ticket is closed" },
}
...
export default function TicketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addMessageMutation = useMutation({
    mutationFn: (data: { content: string }) => api.support.addMessage(resolvedParams.id, data),
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status: string) => api.support.updateTicketStatus(resolvedParams.id, status as any),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {ticket.status !== "CLOSED" && ticket.status !== "RESOLVED" && (
          <Button variant="outline" onClick={handleCloseTicket} disabled={updateStatusMutation.isPending}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="font-medium capitalize">{ticket.priority?.toLowerCase() || "Not set"}</p>
              </div>
