...
### 2026-06-18 — Phase 7.10: Email Verification Flow (closes the Phase 7.8 #25 loop)

**Phase 7.8 #25 shipped the AuthGuard email-verification gate as a one-way trapdoor.** Recon for the planned "frontend 403 banner" follow-up surfaced a substantive gap: `packages/auth/src/index.ts` had `emailAndPassword: { enabled: true }` with NO `emailVerification: {...}` block, NO `sendVerificationEmail` handler, NO `requireEmailVerification` flag. `grep emailVerified apps/api/src packages/auth/src` showed zero writes — nothing in the codebase ever set the flag to `true`. Email/password signups (self-serve at `apps/portal/src/app/page.tsx:45`) created users with `emailVerified: false` forever and were locked out of every state-changing operation indefinitely.

Phase 7.10 closes the loop end-to-end in one PR: backend verification flow (signup → email → click → verified) + frontend banner + resend button + AuthGuard cache invalidation on the verification transition.

**Branch**: `phase-7.10-email-verification-flow` — 4 commits + bedrock-update commit, all tests green.

| Layer | What landed | Commits |
|---|---|---|
| **Better Auth factory** | `AuthFactoryOptions` expanded with `sendEmail(args)` + `onEmailVerified(userId)` opt-in hooks. When `sendEmail` is supplied, the `emailVerification` block is registered: `sendVerificationEmail` wraps the supplied hook with the standard template; `sendOnSignUp: true` auto-sends on signup (THE motivating fix); `autoSignInAfterVerification: true` smoother UX; `expiresIn: 86400` (24h, generous for "let it sit overnight" UX without real risk — single-use tokens). `afterEmailVerification(user)` invokes `onEmailVerified(user.id)` — Phase 0a verified this purpose-built callback fires on the verification transition (simpler than `databaseHooks.user.update.after` — no previous-row inspection needed). Back-compat singleton (`auth = createAuth()`) stays without `sendEmail` since AuthGuard only reads sessions. Test seam `buildAuthOptions(opts)` extracted so the spec can inspect the options passed to `betterAuth()` without standing up a Better Auth runtime. | `77aeb99` |
| **Verification template** | `packages/auth/src/email-templates/verification.ts` — plain inline HTML, no template engine, matches the worker's untemplated approach. Local 3-line `escapeHtml` + `escapeAttr` helpers (no new deps). 12-case spec at `packages/auth/src/__tests__/verification-template.spec.ts` includes 3 XSS cases: `<script>` in user name, `"`-break-out attempt in URL, HTML entities in URL plain-text fallback. | `77aeb99` |
| **API + worker wiring** | `apps/api/src/main.ts` declares a lazy `queueServiceRef: QueueService \| null`. The Better Auth handler is mounted on express BEFORE `NestFactory.create()` (line ~462), so the DI container doesn't exist yet when `createAuth({...})` runs. `sendEmail` closure throws loudly if a request arrives before NestFactory boot completes (impossible in practice). `onEmailVerified` directly calls `invalidateAuthContext(userId)` (sync, no DI deps). `queueServiceRef = app.get(QueueService)` runs immediately after `NestFactory.create()` returns, before `app.listen()`. `apps/worker/src/processors/email.processor.ts` adds `case "send-verification-email"` log tag — the existing default branch would work, but explicit tagging makes per-job-name log filters useful in operator runbooks. | `882fc99` |
| **Frontend** | Split design per planning feedback: presentational shell at `packages/ui/src/components/email-verification-banner.tsx` (no Better Auth / auth context / toast — takes `email + sending + cooldownSeconds + onResend + optional message`); app-specific wiring at `apps/portal/src/components/email-verification-banner-container.tsx` (uses `useAuth`, hits Better Auth's `/api/v1/auth/send-verification-email` via raw fetch — portal's established pattern in `lib/auth.tsx:94,125,144`). 60s client cooldown stacks on top of Phase 7.8 #26's per-IP + per-`SHA-256(email)` server-side rate limit. Short-circuits to null for non-CUSTOMER / verified / signed-out users. Mounted in `apps/portal/src/app/dashboard/layout.tsx` above `<main>`. 6-case Vitest spec on the shell at `packages/ui/src/components/__tests__/email-verification-banner.test.tsx` (ready label, sending disabled, cooldown disabled + not clickable, click invokes onResend once, role=status, custom message override). | `b0bd628` |
| **Integration spec** | `apps/api/src/__tests__/phase-7-10-email-verification.spec.ts` — 10 cases: 8 cover the `buildAuthOptions` wiring (back-compat singleton has no emailVerification block, sendEmail registers the callback, URL is embedded in HTML body, sendOnSignUp/autoSignIn/expiresIn defaults, onEmailVerified registers afterEmailVerification with correct userId, no afterEmailVerification when only sendEmail supplied) + 2 cover the cache-invalidation chain (populates cache → fires onEmailVerified → next lookup misses; per-user isolation regression guard). | `[commit 4]` |

**Pre-impl Phase 0 spike verifications** (the planning step that surfaced the right hooks before code landed): grep of `@better-auth/core/dist/types/init-options.d.mts:528` confirmed `emailVerification.afterEmailVerification(user, request?)` is purpose-built for the verification transition — chose it over `databaseHooks.user.update.after`. Grep of `better-auth/dist/api/routes/email-verification.mjs:40-42` confirmed `/send-verification-email` accepts `{ email: string, callbackURL?: string }` per its Zod body schema — plan's call site is correct. Also caught a plan bug: `sendOnSignUp` belongs in `emailVerification`, not `emailAndPassword` (fixed during commit 1).

**Test count outcome**: `apps/api` jest **39 suites / 547 tests → 40 suites / 557 tests** (+10 from the new Phase 7.10 spec). `packages/ui` Vitest **5 suites / 40 tests → 6 suites / 46 tests** (+6 from the new banner spec). `packages/auth` jest **8 → 20 tests** (+12 from the new verification-template spec — landed with commit 1). Zero regressions.

**Architecture impact**: zero structural changes. New auth factory option (purely additive — `sendEmail` and `onEmailVerified` are optional, the back-compat singleton path is unchanged). New worker job name (`send-verification-email`) plugs into the existing email queue rails. New UI component in `packages/ui`. The AuthGuard's auth-context-cache module is unchanged — `invalidateAuthContext` was already exported, Phase 7.10 just calls it from a new place.

**End-to-end HTTP test deferred**: the plan's case (e) (unverified → resend → verify-link → immediate-protected-POST chain via fresh supertest requests) was scoped down to function-level unit tests because the repo doesn't have Nest+supertest HTTP integration infrastructure today. Each link IS proven individually in the unit spec (`sendEmail` fires, `afterEmailVerification` fires, cache invalidates, per-user isolation holds), and the manual smoke step in the PR body covers the full HTTP chain in dev. Building the harness is captured as Phase 7.10.2 in `bedrock/Work/backlog.md` — unlocks similar tests for AuthGuard / RBAC / queue-replay.

**No scorecard delta** — this is a Phase 7.8 #25 closure-completion (the gate now has a working recovery path), not a new finding closure. Dashboard stays at 25/31 closed.
...
**Mandatory pre-merge GET-mutation audit** (merge blocker before #25 lands): a repository grep for GET route decorators containing mutation-like verbs returned 5 matches — all false-positive substrings (`publish` inside `publisher`, `set` inside `settlements`, `approve` inside `force-approved`); every handler is a pure read operation. The "GETs stay open" assumption in the gate is safe. Audit passes; the PR description retains the exact command and output under `## GET-mutation audit`.
