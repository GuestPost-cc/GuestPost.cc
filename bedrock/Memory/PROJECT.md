...
- Publisher article submission requires both publisher OWNER/MEMBER role authorization and order ownership, and article bodies use one 200,000-character ceiling across controller and service boundaries. A delivery-verification optimistic-concurrency conflict is retryable queue failure, not a successful skip, so concurrent order transitions cannot silently strand verification.
- Platform-revenue reconciliation reports the largest absolute discrepancy between order gross, recorded revenue gross, and the fee/net split; discrepancy rows never report the gross amount as though it were the drift.
- `docs/ORDER_LIFECYCLE_AND_RECONCILIATION.md` is the durable contract for order creation, both fulfillment channels, role-safe visibility, immutable article provenance, atomic transitions, lifecycle presentation, financial invariants, reconciliation, incident diagnosis, and deployment order.
