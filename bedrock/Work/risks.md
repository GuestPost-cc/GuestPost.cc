...
| FulfillmentAssignment claim race let two Ops both succeed (#23) | **Phase 7.14** (commits `590d956` + `837c9ff` + `24cdaa5`): partial unique index `FulfillmentAssignment_orderId_active_unique` on `(orderId) WHERE status IN ('ASSIGNED','IN_PROGRESS')` via `CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS`. App-layer: removed findFirst pre-check from `claim()`; all 3 `upsertAssignment` callers (claim/assign/reassign) wrap the call in `try/catch(P2002)` with caller-appropriate `ConflictException` messages. Gate 0.25 enumeration found 3 callers (not 1); Gate 0.5 verified CONCURRENTLY+WHERE combination on prisma@7.8.0 with full predicate coverage. **Closes the 2026-06-15 audit batch at 31/31 (100%).** |
...
| `MarketplaceFavorite` `addFavorite` TOCTOU race (Phase 7.12.1) — two concurrent `addFavorite(user, listing, null)` calls could both pass the `findFirst` check and both insert, producing duplicate NULL-serviceType rows | **Phase 7.13.2A** (commits `2823b6a` + `e3bf908` + `f84b46c`): new `MarketplaceFavorite_uniq_nullsnotdistinct` unique index over `(userId, listingId, serviceType)` with NULLS NOT DISTINCT semantics, alongside the existing NULLS DISTINCT unique. App-layer rewritten to `try { create } catch (P2002) { findFirst }` (Plan B; Gate 0.5 ruled out Plan A `upsert` because Prisma 7's WhereUniqueInput rejects `null` for nullable composite-key parts at runtime). The umbrella Phase 7.13.2 stays open until 7.13.2B drops the original NULLS DISTINCT index via `DROP INDEX CONCURRENTLY` + renames the new one to canonical. |
