...
- [x] **Phase 7.13.2A — `MarketplaceFavorite` NULLS NOT DISTINCT companion unique** ✅ DONE 2026-06-20 (commits `2823b6a` + `e3bf908` + `f84b46c`, PR pending — merge gated on Phase 7.13 + 7.13.1 staging soaks both clean). Closes Phase 7.12.1 TOCTOU race at the DB level. Single `CREATE UNIQUE INDEX CONCURRENTLY ... NULLS NOT DISTINCT` migration alongside the existing NULLS DISTINCT unique; app-layer rewrite at `marketplace.service.ts:1066-1071` from `findFirst + create` to `try { create } catch (P2002) { findFirst }` (Plan B; Gate 0.5 ruled out Plan A `upsert` because Prisma 7's WhereUniqueInput rejects `null` for nullable composite-key parts). schema.prisma NOTE on MarketplaceFavorite documents both indexes between 7.13.2A and 7.13.2B. apps/api jest favorites spec: 15 → 19 cases (4 new static-source assertions for the try/catch shape + migration + NOTE).
- [ ] **Phase 7.13.2B — drop original `MarketplaceFavorite_userId_listingId_serviceType_key` + rename new index to canonical.** Metadata-only follow-up to 7.13.2A. Pre-flight: 7.13.2A's Gate 0.5 confirmed the original on dev is a stand-alone INDEX (no `pg_constraint` row with `contype='u'` for the name) — so `DROP INDEX CONCURRENTLY` is the right operator path (online, no ACCESS EXCLUSIVE lock). Operator MUST re-run pg_constraint check on staging + prod before writing the 7.13.2B plan (Prisma-generated uniqueness has historically differed across migration eras; do not assume dev's shape applies everywhere). Schema.prisma NOTE updated to single-index state. NO app-layer change. **Lands AFTER 7.13.2A soaks clean in production.**
- [ ] **Phase 7.13.2 (umbrella) — NOT marked DONE until 7.13.2B merges.** Between 7.13.2A and 7.13.2B the live DB has both unique indexes (intentional; tiny extra write cost; the new one is strictly stricter so behavior is monotonic).
